// include different tanks' movements and collision
...
var PlayerTank = Tank.extend({
    ...
    fire: function() {
        ...
        if (global.sound) {
            cc.AudioManager.sharedEngine().playEffect(s_fireSound);
        }
    },
    ...
    destroy: function() {
        ...
        if (global.sound) {
            cc.AudioManager.sharedEngine().playEffect(s_destroySound);
        }
    },
    ...
    explode: function() {
        try {
            ...
            spark(this.getPosition(),this.getParent(), 1.2, 0.3);
        } catch (ex) {
...
var RocketTank = NormalEnemyTank.extend({
    fire: function() {
        ...
        if (global.sound) {
            cc.AudioManager.sharedEngine().playEffect(s_missileSound1);
        }
    },
});
...
var MissileTank = NormalEnemyTank.extend({
    ...
    fire: function() {
        ...
        if (global.sound) {
            cc.AudioManager.sharedEngine().playEffect(s_missileSound2);
        }
    },
...
var FlyTank = NormalEnemyTank.extend({
    ...
    destroy: function() {
        ...
        this.removeFromParentAndCleanup(true);
        if (global.sound) {
            cc.AudioManager.sharedEngine().playEffect(s_flyExplosionSound);
        }
    },
});
